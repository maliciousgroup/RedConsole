...
from src.core.registry.Registry import OptionRegistry, CommandRegistry, ModuleRegistry
...
class RedConsole(object):
    ...
    def print_numbers(self) -> None:
        pq = self.print_queue
        number_of_commands = len(self.command_register.dump_register())
        number_of_modules = len(self.module_register.dump_register())
    ...
    async def command_interpreter(self, data: str) -> None:
        module_list = self.module_register.dump_register()
        command_list = self.command_register.dump_register()
    ...
    async def main(self) -> None:
        ...
        for sig in signals:
            try:
                ...
                pass
        with patch_stdout():
            printer_task = asyncio.create_task(self.print_processor())
